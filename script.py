...
# ========================= 1️⃣ BLOOD, URINE & THYROID TEST ANALYSIS =========================
...
y_blood = label_encoder.fit_transform(y_blood)  # Convert disease labels to numbers
...
def validate_blood_test(values):
    ...
    limits = {
        ...
        "Platelet_Count": (100000, 450000),  # per µL
        "White_Blood_Cells": (2500, 15000),  # per µL
    ...
    # Check if values are within the defined limits
...
def analyze_blood_test(values):
    ...
    # Validate test values before passing to model
    validate_blood_test(values)
    
    # Convert input values into a DataFrame with correct feature names
    feature_names = X_blood.columns.tolist()
    values_df = pd.DataFrame([values], columns=feature_names)
    
    # Get prediction probabilities
    probabilities = blood_model.predict_proba(values_df)[0]  # Get probability for each disease
    
    # Find the most confident prediction
    max_prob_index = np.argmax(probabilities)  # Index of highest probability
    confidence = probabilities[max_prob_index] * 100  # Convert to percentage
    predicted_disease = label_encoder.inverse_transform([max_prob_index])[0]  # Convert index to disease name
...
for col in categorical_cols:
    if col in urine_df.columns:
        urine_df[col] = urine_df[col].astype(str)  # Ensure all values are strings
        urine_df[col] = LabelEncoder().fit_transform(urine_df[col])  # Convert to numbers
...
def analyze_urine_test(values):
    """Predict disease from urine test values and ensure correct feature order."""
...
# Function to analyze thyroid test
def analyze_thyroid_test(values):
    """Predicts the most confident thyroid disease from test values."""
    
    # Use the exact feature names used during training
    feature_names = X_thyroid.columns.tolist()
    
    # Ensure test data is in the correct order
    values_df = pd.DataFrame([values], columns=feature_names)
    
    # Get prediction probabilities
    probabilities = thyroid_model.predict_proba(values_df)[0]  # Get probability for each disease
    
    # Find the most confident prediction
    max_prob_index = np.argmax(probabilities)  # Index of highest probability
    confidence = probabilities[max_prob_index] * 100  # Convert to percentage
    predicted_disease = label_encoder_thyroid.inverse_transform([max_prob_index])[0]  # Convert index to disease name
...
#  Define Pre-processing Functions for Each Model
...
# ===================== SPECIALIZED DETECTION MODELS (STAGE 2) =====================
...
def full_cancer_detection(image_path):
    """Runs full pipeline: classification + specialized detection."""
    ...
    # Run final detection using the identified model
...
def medical_chatbot(question):
    """
    Chatbot using Google Gemini AI with improved formatting for short paragraphs.
    """
...
def format_response(response_text):
    """
    Formats AI-generated responses into short paragraphs, fixes bullet points, and improves readability.
    """

    #  Remove unnecessary Markdown formatting
    formatted_text = response_text.replace("**", "").replace("*", "")

    #  Standardize section headers and structure
    replacements = {
    ...
    #  Fix spacing issues and ensure proper line breaks
    formatted_text = re.sub(r"\n{2,}", "\n\n", formatted_text)  # Prevent extra blank lines
...
def find_nearest_hospital(location):
    """Find hospitals using OpenStreetMap Nominatim API"""
...
if __name__ == "__main__":
    print(analyze_blood_test([30, 1, 13.5, 250000, 7000, 4.8, 90, 32, 34, 100, 1.2, 2.5]))  # Correct number of inputs
    print(analyze_urine_test([30, 1, 2, 100, 0, 1, 0, 1, 1.02, 6.5]))  #  Correct number of inputs
    test_thyroid_values = [30, 1, 2.5, 1.8, 4.5, 30, 50, 1.2]  #  Define the test values
    print(analyze_thyroid_test(test_thyroid_values))  #  Call function properly
